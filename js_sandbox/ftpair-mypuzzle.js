...
function ftpair_mp_getVars(){
    ...
    m_uriBase_ftpair = jQuery('#var_uribase_ftpair').text();
    m_folderGallery_ftpair = jQuery('#var_galleryFolder_ftpair').text();
    m_pathGallery_ftpair = jQuery('#var_galleryPath_ftpair').text();
...
function ftpair_mp_init_grid()
{
    ...
    m_images = new Array();
    var sUrl = m_uriBase_ftpair+'index.php?option=com_mp_ftpair&view=getcards&format=json&path='+m_folderGallery_ftpair+'&cards='+m_cards;
    ...
    jQuery.getJSON(sUrl,'callback=?', function(data){
        ...
        for (var x = 0; x < m_images.length; x++){
            ...
            item.attr({'style': ''});
            item.find('img').attr('src',m_galleryDir+'/'+m_images[x]);
            item.click(function(){ 
